...
export const PeliculasController = {
  ...
  try {
    const { titulo } = req.params;
    const peliculas = await PeliculaModel.buscarTitulo(req.db,titulo);
    res.json(peliculas);
  } catch (err) {
    console.error("Error en buscarPorTitulo:", err);
  ...
  async getPeliculasPorCategoria(req, res) {
    try {
      const { categoria } = req.params;
      const peliculas = await PeliculaModel.buscarPorCategoria(categoria);

      res.json(peliculas);
    } catch (err) {
      console.error("Error en getPeliculasPorCategoria:", err);
